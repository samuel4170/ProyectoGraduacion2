...
export default function EspecialidadForm(){
  ...
  const handleSubmit = async e => {
    ...
    if (editing) {
      await fetch(`http://localhost:8080/api/especialidades/${params.IdEspecialidad}`, {
        method: "PUT",
        headers: {
      ...
      await fetch("http://localhost:8080/api/especialidades", {
        method: "POST",
        body: JSON.stringify(Especialidad),
  ...
  const loadEspecialidad = async (IdEspecialidad) => {
    const res = await fetch(`http://localhost:8080/api/especialidades/${IdEspecialidad}`)
