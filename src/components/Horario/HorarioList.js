...
export default function HorarioList() {
  ...
  useEffect(() => {
    loadHorarios();
    fetch('http://localhost:8080/api/dias') // Cargar días
  ...
  >
    ...
    >
      <div style={{ color: 'white' }}>
        ...
        <Typography>{`Hora de Fin: ${horario.HoraFin}`}</Typography>
        <Typography>{`Día: ${
          dias.find((dia) => dia.IdDia === horario.IdDia)?.NombreDia || 'Día no especificado'
        }`}</Typography> 
      </div>
